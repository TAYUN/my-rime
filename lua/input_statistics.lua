...
-- 汉字频率记录
-- -----------------------------------------------------------------------------
local function record_character_frequency(text)
    if not ensure_db_open() then return end
    
    for char in text:gmatch("[\228-\233][\128-\191][\128-\191]") do
        local code = utf8.codepoint(char)
        if is_chinese_code(code) then
            local key = "hz_" .. string.format("U%04X", code)
            db:update(key, tostring(db_get(key) + 1))
        end
    end
end
...
-- 高频汉字查询
-- -----------------------------------------------------------------------------
local function get_top_characters(limit)
    if not ensure_db_open() then return {} end
    ...
    db:query_with("", function(key, value)
        if key:sub(1, 3) == "hz_" and #key == 8 then
            local hex = key:sub(5)
            local code = tonumber(hex, 16)
            local count = tonumber(value) or 0
            if count > 0 then
                table.insert(results, {char = utf8.char(code), count = count})
            end
        end
    end)
...
local function init(env)
    ...
    env.stat_notifier = ctx.commit_notifier:connect(function(ctx)
        ...
        record_character_frequency(commit_text)
    end)
end
...
local function translator(input, seg, env)
    ...
    if pzhl_limits[input] then
        local limit = pzhl_limits[input]
        local ok = pcall(function()
            local top_chars = get_top_characters(limit)
            if #top_chars > 0 then
                local chars = {}
                for _, item in ipairs(top_chars) do
                    table.insert(chars, item.char)
                end
                local content = table.concat(chars, "")
                
                -- 使用绝对路径，确保用户能找到
                local file_path = "C:\\Users\\WY\\AppData\\Roaming\\Rime\\rime_high_freq.txt"
                local f = io.open(file_path, "w")
                if f then
                    f:write(content)
                    f:close()
                    yield(Candidate("stat", seg.start, seg._end, "※ 已导出 TOP " .. #top_chars .. " 到 " .. file_path, "💾"))
                else
                    yield(Candidate("stat", seg.start, seg._end, "※ 导出失败：无法写入 " .. file_path, "❌"))
                end
            else
                yield(Candidate("stat", seg.start, seg._end, "※ 暂无汉字频率数据", "📝"))
            end
        end)
        
        if not ok then
            yield(Candidate("stat", seg.start, seg._end, "※ 导出出错", "❌"))
        end
        return
    end
    ...
    if hzpl_limits[input] then
        local limit = hzpl_limits[input]
        local ok = pcall(function()
            local top_chars = get_top_characters(limit)
            if #top_chars > 0 then
                local result = "※ 高频汉字 TOP " .. #top_chars .. "\n───────────────\n"
                for i, item in ipairs(top_chars) do
                    local rank = string.format("%2d", i)
                    local char = item.char
                    local count = string.format("%5d", item.count)
                    result = result .. "  " .. rank .. ". " .. char .. "  " .. count .. "次\n"
                end
                yield(Candidate("stat", seg.start, seg._end, result, "🔥"))
            else
                yield(Candidate("stat", seg.start, seg._end, "※ 暂无汉字频率数据（请先输入一些汉字）", "📝"))
            end
        end)
        
        if not ok then
            yield(Candidate("stat", seg.start, seg._end, "※ 查询失败", "❌"))
        end
        return
    end
